...
static void LancerMenuPrincipal()
{
    ...
    while (true)
    {
        ...
        if (selection == 0)
        {
            // Lancer le jeu et ne jamais revenir au menu principal
            LancerJeu();
            return; // Quitte la boucle/menu principal après le jeu
        }
        else if (selection == 1)
        {
            // Afficher les règles puis revenir au menu
            AfficherRegles();
        }
    }
}
...
static void LancerJeu()
{
    ...
    // Ici, tu pourras ajouter la logique réelle du jeu

    Console.WriteLine("Appuyez sur une touche pour quitter le programme.");
    Console.ReadKey(true);

    // Fin du programme ici, on ne revient pas au menu
}
...
static int AfficherMenu(string[] options)
{
    int selection = 0;
    ConsoleKey key;
